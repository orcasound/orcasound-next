...
export default function WaveformPlayer({ audioUrl }: WaveformPlayerProps) {
  ...
  useEffect(() => {
    ...
    waveSurferRef.current.on("ready", () => {
      ...
      if (containerRef.current) {
        ...
        setZoomLevel(containerW / minZoom); // start fully zoomed out
        waveSurfer.zoom(containerW / minZoom);
  ...
  useEffect(() => {
    const observer = new ResizeObserver(() => {
      ...
      if (containerRef.current && lastSecondsInWindowRef.current) {
        ...
        setZoomLevel(containerW / clampedZoom);
  ...
  return (
    <div className="waveform-player" style={{ width: "100%" }}>
      ...
      <div>Zoom percent: {(zoomPercent * 100).toFixed(0)}%</div>
