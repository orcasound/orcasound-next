...
Scope for this repo:
- `orcasound-next` is the active implementation context.
- `orcasite` is the convention anchor for production-aligned frontend patterns.
- `esp32_ui` is a parallel track in a separate repository with mirrored document structure.
...
- Migration preference: generated GraphQL hooks over ad-hoc GraphQL hooks where equivalent operations exist
...
- Map current data paths to target typed hook boundaries.
